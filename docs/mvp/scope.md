...
- [x] **1 role duy nhất** — chọn Backend Dev *hoặc* BA, không cả 2
...
## Chọn role nào cho MVP?

Quyết định dựa trên data availability:

| Role | Chọn nếu |
|---|---|
| **Backend Dev** | Repo có ≥ 30 merged MRs / 90d, review comments dồi dào, có CI |
| **Business Analyst** | Confluence có ≥ 20 spec pages / 90d với linked Jira issues |

Nếu cả 2 đều OK → ưu tiên **role có nhiều "negative signals" rõ** (revert, reopen,
scope change) vì distill có pattern để học từ failure.

Trong tài liệu tôi sẽ ví dụ với **Backend Dev** (default), nhưng skill-pack-spec
cover cả 2.
...
| 0/3 | ❌ Pivot: đổi data source / đổi role / abandon |
...
| A3 | Confluence space "Product" có ≥ 20 specs (nếu chọn BA) | CQL query |
| A4 | Repo pilot có ≥ 30 merged MRs / 90d (nếu chọn dev) | GitLab API |
| A5 | LLM API key + budget OK | `anthropic` SDK test |

Nếu A1–A2 fail → MVP blocked. Nếu A3 hoặc A4 fail → đổi role tương ứng.
